...
-- ====== ULTRA SMOOTH Animation Controller v5.0 ======
...
function AnimationController.new(character)
    ...
    -- Ultra smooth buffers
    self.velocityBuffer = {}
    self.velocityBufferSize = 12
    self.positionBuffer = {}
    self.positionBufferSize = 8
    self.rotationBuffer = {}
    self.rotationBufferSize = 6
    
    -- Advanced terrain detection
    self.terrainHistory = {}
    self.terrainHistorySize = 20
    self.slopeAngle = 0
    self.lastGroundY = nil
    self.isOnGround = false
    self.groundCheckBuffer = {}
    
    -- State detection
    self.stateHistory = {}
    self.stateHistorySize = 15
...
function AnimationController:calculateTerrainSlope()
    if #self.terrainHistory < 2 then
        return 0
    end
    ...
    for i = 2, #self.terrainHistory do
        local prev = self.terrainHistory[i-1]
        local curr = self.terrainHistory[i]
        
        local yDiff = curr.Y - prev.Y
        local horizontalDist = math.sqrt((curr.X - prev.X)^2 + (curr.Z - prev.Z)^2)
        
        if horizontalDist > 0.1 then
            local angle = math.deg(math.atan2(yDiff, horizontalDist))
            sumSlope = sumSlope + angle
            count = count + 1
        end
    end
    
    return count > 0 and (sumSlope / count) or 0
end
...
function AnimationController:smoothMoveTo(targetCF, velocity)
    ...
    -- Ultra smooth velocity
    table.insert(self.velocityBuffer, velocity)
    if #self.velocityBuffer > self.velocityBufferSize then
        table.remove(self.velocityBuffer, 1)
    end
    
    local smoothedVelocity = Vector3.new()
    for _, vel in ipairs(self.velocityBuffer) do
        smoothedVelocity = smoothedVelocity + vel
    end
    smoothedVelocity = smoothedVelocity / #self.velocityBuffer
    ...
    table.insert(self.terrainHistory, avgPosition)
    if #self.terrainHistory > self.terrainHistorySize then
        table.remove(self.terrainHistory, 1)
    end
    
    self.slopeAngle = self:calculateTerrainSlope()
    
    -- Apply movement with higher smoothness
    local smoothCF = CFrame.lookAt(avgPosition, avgPosition + avgLookVector)
    self.rootPart.CFrame = self.rootPart.CFrame:Lerp(smoothCF, 0.45)
...
function AnimationController:detectState(velocity, recordedState)
    ...
    self.isOnGround = self:isCharacterGrounded()
    
    local currentY = self.rootPart.Position.Y
    if self.lastGroundY then
        table.insert(self.groundCheckBuffer, currentY - self.lastGroundY)
        if #self.groundCheckBuffer > 10 then
            table.remove(self.groundCheckBuffer, 1)
        end
    end
    self.lastGroundY = currentY
    
    local avgYMovement = 0
    if #self.groundCheckBuffer > 0 then
        local sum = 0
        for _, diff in ipairs(self.groundCheckBuffer) do
            sum = sum + diff
        end
        avgYMovement = sum / #self.groundCheckBuffer
    end
    
    local detectedState = "Idle"
    
    -- PRIORITY 1: Horizontal movement on ground (prevents false falling)
    if self.isOnGround and horizontalSpeed > 2 then
        if math.abs(self.slopeAngle) < 45 then
            ...
            if horizontalSpeed > 15 then
                detectedState = "Running"
            else
                detectedState = "Walking"
            end
        end
    -- PRIORITY 2: Jump detection
    elseif verticalSpeed > 25 and not self.isJumping then
        self.isJumping = true
        self.jumpStartTime = tick()
        detectedState = "Jumping"
    -- PRIORITY 3: Fall detection (only when truly falling)
    elseif not self.isOnGround and verticalSpeed < -35 and not self.isJumping then
        if horizontalSpeed < 3 then
            detectedState = "Falling"
        else
            -- Still consider as running if has horizontal momentum
            detectedState = "Running"
        end
    ...
    -- Check if jump duration passed
    if self.isJumping and (tick() - self.jumpStartTime) > 0.6 then
        self.isJumping = false
    end
    
    -- Ultra smooth state detection with history
    table.insert(self.stateHistory, detectedState)
    if #self.stateHistory > self.stateHistorySize then
        table.remove(self.stateHistory, 1)
    end
    
    -- Get dominant state from history
    local stateCount = {}
    for _, s in ipairs(self.stateHistory) do
        stateCount[s] = (stateCount[s] or 0) + 1
    end
    
    local dominantState = detectedState
    local maxCount = 0
    ...
    -- Use recorded state as fallback if available
    if recordedState and recordedState ~= "" and recordedState ~= "Idle" then
...
function AnimationController:applyState(state, forceJump)
    if self.currentState == state or not self.humanoid then return end
    ...
    elseif state == "Falling" then
        -- Only apply freefall if truly in air
...
subtitleLabel.Text = "Ultra Smooth • Fixed Falling • Terrain Detection"
...
statusDesc.Text = "Ultra Smooth • Frames: 0"
...
local function updateStatus(state, animation, frames)
    ...
    statusDesc.Text = string.format("Ultra Smooth • Frames: %d", frames or 0)
            ...
            if humanoidRootPart and humanoidRootPart.Parent and character and character.Parent and currentReplayToken == token and bodyRecorder and humanoid then
                ...
                pcall(function()
                    if animController then
                        ...
                        humanoidRootPart.CFrame = humanoidRootPart.CFrame:Lerp(targetCFrame, 0.45)
                        humanoidRootPart.AssemblyLinearVelocity = velocity
                    end
                    
                    if frame.BodyFrame and frame.BodyFrame.Motors then
                        bodyRecorder:applyFrame(frame.BodyFrame, 0.5)
                    end
                    
                    if animController then
                        animController:applyState(playbackState, false)
                    end
                end)
...
print("╔═══════════════════════════════════════════╗")
print("║  STRIDEX v5.0 - ULTRA SMOOTH EDITION      ║")
print("╚═══════════════════════════════════════════╝")
print("")
print("✅ DEVICE VERIFIED!")
print("✅ NEW FEATURES v5.0:")
print("   • 🌊 ULTRA SMOOTH Movement (12 buffers)")
print("   • 🏔️ Advanced Terrain Detection")
print("   • 🔧 FIXED False Falling Detection")
print("   • 🎯 Smart Ground Detection")
print("   • 📐 Slope Angle Calculation")
print("   • 🛡️ God Mode Protection")
print("")
print("✅ IMPROVEMENTS:")
print("   • 3x larger velocity buffer (12 frames)")
print("   • 2x larger position buffer (8 frames)")
print("   • 2x larger rotation buffer (6 frames)")
print("   • Enhanced terrain history (20 frames)")
print("   • Priority-based state detection")
print("   • Raycast ground detection")
print("   • 0.45 lerp for ultra smooth movement")
print("   • 0.5 body frame smoothness")
print("")
print("🎮 CONTROLS:")
...
print("")
print("🏔️ TERRAIN HANDLING:")
print("   • Uphill: Maintains Running state")
print("   • Downhill: Maintains Running state")
print("   • Flat: Perfect Running state")
print("   • Jump: Natural jump animation")
print("   • Fall: Only when truly falling")
print("")
print("🎮 READY TO USE - ULTRA SMOOTH!")
